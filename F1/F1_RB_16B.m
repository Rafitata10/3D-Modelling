...
stlFile = 'C:/Users/rafar/OneDrive - Universidad de Málaga/Escritorio/University/Matlab/F1/sources/Red_Bull_RB16B.stl';
...
figure('Name', 'RB16B F1 car 3D Model Original', 'NumberTitle', 'off', 'Color', [1 1 1]);
...
ylabel('Y');
zlabel('Z');
title('Modelo 3D original del F1 RB16B');
...
if isempty(carasCortadasY) || all(carasCortadasY == 0)
    ...
    ylabel('Y');
    zlabel('Z');
    title(['Modelo F1 RB16B cortado en Y (Plano Y-Z) = ', num2str(cutPlaneY)]);
    lighting gouraud;
    camlight headlight;
    material shiny;
    rotate3d on;
...
if isempty(carasCortadasZ) || all(carasCortadasZ == 0)
    ...
    ylabel('Y');
    zlabel('Z');
    title(['Modelo F1 RB16B cortado en Z (Plano X-Z) = ', num2str(cutPlaneZ)]);
    lighting gouraud;
    camlight headlight;
    material shiny;
    rotate3d on;
